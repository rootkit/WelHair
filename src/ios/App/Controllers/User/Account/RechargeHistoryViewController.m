...
@interface RechargeHistoryViewController () <UITableViewDataSource, UITableViewDelegate>
...
{
    static NSString *cellIdentifier = @"DepositCellIdentifier";
    DepositCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[DepositCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.contentView.backgroundColor = cell.backgroundColor = [UIColor clearColor];
    }

    Deposit *deposit = [self.datasource objectAtIndex:indexPath.row];
    [cell setup:deposit];

    return cell;
}

#pragma mark Deposit Search API
...
- (void)finishGetDeposits:(ASIHTTPRequest *)request
{
    NSDictionary *rst = [Util objectFromJson:request.responseString];
    NSInteger total = [[rst objectForKey:@"total"] integerValue];
    NSArray *dataList = [rst objectForKey:@"deposits"];
    ...
    for (NSDictionary *dicData in dataList) {
        [arr addObject:[[Deposit alloc] initWithDic:dicData]];
    }
...
- (void)failGetDeposits:(ASIHTTPRequest *)request
{
}
