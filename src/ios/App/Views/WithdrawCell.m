...
@property (nonatomic, strong) Withdraw *withdraw;
...
- (void)setup:(Withdraw *)withdraw
{
    ...
    self.titleLbl.text = [NSString stringWithFormat:@"提现【%@】", self.withdraw.withdrawNo];
    self.amountLbl.text = [NSString stringWithFormat:@"%.2f", self.withdraw.amount];
    self.datetimeLbl.text = [[NSDate dateWithHMFormatter] stringFromDate:self.withdraw.createdDate];
    switch (self.withdraw.status) {
        case WHWithdrawStatusPending:
            self.statusLbl.hidden = NO;
            self.statusLbl.text = @"审核中";
            break;
        case WHWithdrawStatusSuccess:
            self.statusLbl.hidden = NO;
            self.statusLbl.text = @"审核通过";
            break;
        case WHWithdrawStatusFailed:
            self.statusLbl.hidden = NO;
            self.statusLbl.text = @"审核拒绝";
            break;
        default:
            break;
    }
