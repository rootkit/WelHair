...
- (void)setup
{
	...
	if (kAllowsMedia)
	{
		...
		CGFloat yHeight = (inputViewHeight - frame.size.height) / 2.0f;
    ...
    if (kAllowsMedia) {
		...
		CGRect frame = inputView.sendButton.frame;
		frame.size.width -= 16;
		frame.origin.x += 16;
		inputView.sendButton.frame = frame;

		// add the camera button
		[inputView addSubview:mediaButton];

		// move the tet view over
		frame = inputView.textView.frame;
		...
		frame.size.width += 16;
		inputView.textView.frame = frame;
	}
